...
def write_sumo_nodes(df, filename):
    ...
    with open(filename, "w") as f:
        ...
        for _, row in df.iterrows():
            ...
            coord_map[node_id] = (x, y)
            f.write(f'  <node id="{node_id}" x="{x:.2f}" y="{y:.2f}" />\n')
        f.write("</nodes>\n")
        f.write('</network>\n')

    return coord_map

def calculate_edge_lengths(edge_df, coord_map):
    edge_df.columns = [c.strip().lower() for c in edge_df.columns]
    distances = []

    for _, row in edge_df.iterrows():
        from_node = str(row["init_node"])
        to_node = str(row["term_node"])
        try:
            ...
            print(f"⚠️ Skipping edge {from_node} → {to_node}: {e}")
...
coord_map = write_sumo_nodes(siouxfallsnode, "nodes.nod.xml")
siouxfallsnet_with_lengths = calculate_edge_lengths(siouxfallsnet, coord_map)
write_sumo_edges(siouxfallsnet_with_lengths, "edges.edg.xml")
